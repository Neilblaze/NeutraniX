// Build file 
// Copyright (c) 2021, Pratyay Banerjee
// Co-Author: Nilanjan Ray (Department of Computing Science, University of Alberta, CA)
...
function withCameraStream(e)
{
	if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia)
	{
		...
		{
			video: true
		}).then(e)["catch"](() => alert(`Check if camera permission is enabled or not!`))
	}
}
...
function simplifyFaceModel(e)
{
	...
	const c = [0, 1];
	return {
		face: c.map(e => (t[e] + n[e]) * .5),
		eyes: c.map(e => (a[1][e] + a[0][e]) * .5),
		ears: c.map(e => (a[5][e] + a[4][e]) * .5)
	}
}
